...
def get_Qwen_dynamic_headers(chat_id: str = "") -> Dict[str, str]:
    """
    生成 qwen.ai 特定的动态浏览器 headers，包含随机 User-Agent
    使用通用的 UserAgent 工具，但添加 qwen.ai 特定的业务逻辑

    Args:
        chat_id: 聊天 ID，用于生成正确的 Referer

    Returns:
        Dict[str, str]: 包含 qwen.ai 特定配置的 headers
    """
...
def get_auth_token_sync() -> str:
    ...
    if settings.ANONYMOUS_MODE:
        try:
            headers = get_Qwen_dynamic_headers()
            with httpx.Client() as client:
                response = client.get("https://chat.qwen.ai/api/v1/auths/", headers=headers, timeout=10.0)
...
class QwenTransformer:
    ...
    async def transform_request_in(self, request: Dict[str, Any]) -> Dict[str, Any]:
        """
        转换OpenAI请求为qwen.ai格式
        整合现有功能：模型映射、MCP服务器等
        """
        logger.info(f"🔄 开始转换 OpenAI 请求到 qwen.ai 格式: {request.get('model', settings.PRIMARY_MODEL)} -> qwen.ai")
        ...
        # 构建请求配置
        dynamic_headers = get_Qwen_dynamic_headers(chat_id)
