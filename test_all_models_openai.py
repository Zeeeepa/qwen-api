#!/usr/bin/env python3
"""
Test ALL models using OpenAI Python client
"""
...
MODELS_TO_TEST = [
    ...
    # Model mappings (should work via smart mapping)
    "gpt-4",
    "gpt-3.5-turbo",
    "claude-3-opus",
    "claude-3-sonnet",
...
def test_model(model_name: str, test_number: int, total: int) -> bool:
    ...
    try:
        ...
        print(f"   Response: {Colors.CYAN}{content[:100]}{Colors.RESET}")
...
def main():
    ...
    print(f"🤖 TESTING ALL MODELS WITH OPENAI CLIENT")
    print(f"{'='*80}{Colors.RESET}\n")
    
    print(f"{Colors.CYAN}Server: {BASE_URL}{Colors.RESET}")
    print(f"{Colors.CYAN}Total Models: {len(MODELS_TO_TEST)}{Colors.RESET}")
    ...
    for i, model in enumerate(MODELS_TO_TEST, 1):
        success = test_model(model, i, len(MODELS_TO_TEST))
    ...
    print(f"{Colors.BOLD}Total Models Tested:{Colors.RESET} {len(MODELS_TO_TEST)}")
    print(f"{Colors.GREEN}✅ Passed:{Colors.RESET} {passed}")
    print(f"{Colors.RED}❌ Failed:{Colors.RESET} {failed}")
    
    # Pass rate
    pass_rate = (passed / len(MODELS_TO_TEST) * 100) if len(MODELS_TO_TEST) > 0 else 0
    ...
    if pass_rate == 100:
        print(f"\n{Colors.GREEN}{Colors.BOLD}🎉 ALL MODELS WORKING! 🎉{Colors.RESET}\n")
    elif pass_rate >= 80:
        print(f"\n{Colors.YELLOW}{Colors.BOLD}⚠️ Most models working ({pass_rate:.1f}%){Colors.RESET}\n")
    elif pass_rate >= 50:
        print(f"\n{Colors.YELLOW}{Colors.BOLD}⚠️ Some models working ({pass_rate:.1f}%){Colors.RESET}\n")
    else:
