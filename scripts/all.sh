...
bash "$SCRIPT_DIR/start.sh" > /tmp/qwen_server.log 2>&1 &
SERVER_PID=$!
...
bash "$SCRIPT_DIR/send_request.sh" "qwen-max-latest" "What is 2+2?"
echo ""
sleep 2

# Send second test request
log_info "Test 2: Code help question"
bash "$SCRIPT_DIR/send_request.sh" "qwen-max-latest" "Can you help me fix my code??"
echo ""
